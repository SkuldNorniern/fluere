...
struct FlowState {
    ...
    ///
    /// First rather than last, and never overwritten: a router sending
    /// unreachables to one host quotes a different datagram each time, and
...
fn fold(
    ...
    // The first quote only. Later errors on this flow may name other flows, and
