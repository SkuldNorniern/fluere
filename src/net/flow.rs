...
pub fn flow_convert(packet: pcap::Packet) -> Result<(Key,V5Record), NetError> {
    ...
    let src_port = match i.get_next_level_protocol() {
        ...
        _ => return Err(NetError::UnknownProtocol{protocol : i.get_next_level_protocol().to_string()}),
    ...
    let dst_port = match i.get_next_level_protocol() {
        ...
        _ => return Err(NetError::UnknownProtocol{protocol : i.get_next_level_protocol().to_string()}), //panic!("Unknown protocol {:?}", i),
    };
    let mut fin = 0;
    let mut syn = 0;
    let mut rst = 0;
